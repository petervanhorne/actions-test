...
#  ['ring', 'str']
                            ...
                            try:
                                ...
                                result = template.assert_fact(**fact1)
                                if self.control["debug-fact"] == 1:
                                    self.print_log("**** DDR Debug: syslog fact dictionary: " + str(result))
...
#
                                try:
                                    ...
                                    result = template.assert_fact(**fact1)
                                    if self.control["debug-fact"] == 1:
                                        self.print_log("**** DDR Debug: notification fact dictionary: " + str(result))
...
#
            if self.control["show-timing"] == 1:
                ...
                self.timing = {
                "load-initial-facts" : 0.0,
                "load-test-facts" : 0.0,
                "load-ddr-control" : 0.0,
                "get-device-facts" : 0.0,
                "clips-runtime" : 0.0}    
#
...
{3}    </dict-facts>
    ...
    ###################################################################################################
        ...
        try:
            if os.path.exists('/bootflash/guest-share/ddr' + 'ddr-facts'):
                ...
                self.control.update({"show-file" : '/bootflash/guest-share/ddr' + 'ddr-show'})
                self.control.update({"nc-file" : '/bootflash/guest-share/ddr' + 'ddr-nc'})
            else:
                ...
                self.control.update({"show-file" : flist[9]})                
                self.control.update({"nc-file" : flist[10]})                
        ...
        if self.control["debug-fact"] == 1:
            self.print_log("**** DDR Debug: initialize file names: \n" + str(flist))
    ...
    ######################################################################################
    #
    # If use-devices = 1 make connections to devices.  If 0 do not connect to devices
    # and use simulated data from ddr-sim, ddr-show, and ddr-nc files
    #
    ######################################################################################
    ######################################################################################
        if self.control["use-devices"] == 1:
    #
    ...
    ################################################################################################
        ...
        if self.control["sim-show"] == 1:
            try:
                if self.control["debug-show"] == 1:
                    self.print_log("**** DDR Debug: ddr-show file name: " + str(self.control["show-file"]))
                fd = open(self.control["show-file"])
                inputdata = imp.load_source('inputdata', self.control["show-file"], fd)
                self.show_list = inputdata.show_list

                if self.control["debug-show"] == 1:
                    self.print_log("**** DDR Debug: in sim-show content load: " + str(self.show_list))

                fd.close()
            except Exception as e:
                self.print_log('\n%%%% DDR Error: ddr-show file read error: ' + str(e))

        if self.control["sim-nc"] == 1:
            try:
                if self.control["debug-nc"] == 1:
                    self.print_log("**** DDR Debug: ddr-nc file name: " + str(self.control["nc-file"]))
                fd = open(self.control["nc-file"])
                inputdata = imp.load_source('inputdata', self.control["nc-file"], fd)
                self.nc_list = inputdata.nc_list
                fd.close()
            except Exception as e:
                self.print_log('\n%%%% DDR Error: ddr-nc file read error: ' + str(e))
    ...
    #
        try:
            for device in device_dictionary:
                ...
                result = template.assert_fact(**device)
                if self.control["debug-fact"] == 1:
                    self.print_log("\n**** DDR Debug: Assert FACT for each device: " + str(result))
            ...
            pass
    #
    # Assert test-mode FACT to control using ddr-sim, ddr-show, ddr-nc and ddr-rule debugging
    # (test-mode (use-ddr-sim "FALSE") (use-ddr-show "FALSE") (use-ddr-nc "FALSE") (debug "FALSE"))
    #
        try:
            template = self.env.find_template("test-mode")
            test_mode={}
            if self.control["sim-facts"] == 1:
                test_mode["use-ddr-sim"] = "TRUE"
            else:
                test_mode["use-ddr-sim"] = "FALSE"
            if self.control["sim-show"] == 1:
                test_mode["use-ddr-show"] = "TRUE"
            else:
                test_mode["use-ddr-show"] = "FALSE"
            if self.control["sim-nc"] == 1:
                test_mode["use-ddr-nc"] = "TRUE"
            else:
                test_mode["use-ddr-nc"] = "FALSE"
            if self.control["debug-rule"] == 1:
                test_mode["debug"] = "TRUE"
            else:
                test_mode["debug"] = "FALSE"
                
            result = template.assert_fact(**test_mode)
            if self.control["debug-fact"] == 1:
                self.print_log("\n**** DDR Debug: Assert FACT to control simulated FACTs, show and netconf responses: " + str(result))
        except Exception as e:
            if self.control["debug-fact"] == 1:
                self.print_log("%%%% DDR Exception: Initializing test-mode FACT: " + str(e))
            pass
    ...
    def get_template_multifacts_protofact_index(self, fact, hard_slot, hard_slot_value, device_index=None, sim_nc=None, simdata=None):
      '''
     ...
     "protofact": {"template": 'nve-peer-list',
      ...
      try:
        try:
            ...
            self.print_log("\n%%%% DDR Error: get_template_multifacts_protofact_index device object error: " + str(e))
            return
    #
    # Use simulated get response if required
    #
        try:
            if sim_nc == True:
                get_result = simdata
                ...
                get_result = self.device[int(device_index)].get(filter=('subtree', str(fact['path'])))                
                instances = xml.dom.minidom.parseString(get_result.xml).getElementsByTagName(fact["assert_fact_for_each"])
        except Exception as e:
            self.print_log("\n%%%% DDR Error: get_template_multifacts_protofact_index ddr-nc response: " + str(e))
            return
    ...
    def get_template_multifacts_index(self, fact, slot, slot_value, device_index, sim_nc=None, simdata=None):
        '''
        ...
        try:
            try:
                ...
                self.print_log("\n%%%% DDR Error: get_template_multifacts_index device object error: " + str(e))
                return
    ...
    #
            try:
                if sim_nc == True:
                    ...
                    if self.control["debug-nc"] == 1:
                        self.print_log("**** DDR Debug: get_template_multifacts_index: ddr-nc simulated response:\n" + str(simdata))
                else:
                    get_result = self.device[int(device_index)].get(filter=('subtree', path))                
                    instances = xml.dom.minidom.parseString(get_result.xml).getElementsByTagName(key)
            except Exception as e:
                self.print_log("\n%%%% DDR Error: get_template_multifacts_index ddr-nc response: " + str(e))
                return
    ...
    #                   
                try:
                    ...
                    result = template.assert_fact(**fact1)
                    if self.control["debug-fact"] == 1:
                        self.print_log("\n**** DDR Debug: get_template_multifacts_index fact asserted: " + str(result))
                except Exception as e:
                    if self.control["debug-fact"] == 1:
...
#############################################################################
    ...
    #
                if type(value) is str and "+" in value:
                    try:
                        ...
                        if self.control["debug-fact"] == 1:
                            self.print_log("\n**** DDR Debug: assert_template_fact: No value found for slot: " + str(e))
                        continue
    #
    ...
    #
            try:
                ...
                result = template.assert_fact(**fact1)
                if self.control["debug-fact"] == 1:
                    self.print_log("\n**** DDR Debug: assert_template_fact: " + str(result))
            except Exception as e:
                if self.control["debug-fact"] == 1:
    ...
    # Assert the Syslog FACT
                try:
                    ...
                    result = template.assert_fact(**parsed_genie_output)
                    if self.control["debug-fact"] == 1:
                        self.print_log("**** DDR Debug: assert_syslog_fact result: \n" + str(result))                  
                except Exception as e:
    ...
    def assert_statistics_fact(self, template_name, statistics):
        try:
            ...
            try:
                ...
                result = template.assert_fact(**fact)
                if self.control["debug-fact"] == 1:
                    self.print_log("**** DDR Debug: assert_statistics_fact: " + str(result))
            except Exception as e:
                self.print_log("%%%% DDR Error: Exception assert_statistics_fact: " + str(e) + " " + str(fact))
                pass
        except Exception as e:
...
#
    ...
    #
                for fact in self.control["initial-facts-list"]:
                    if self.control["debug-fact"] == 1:
                        self.print_log("**** DDR Debug: Assert initial_fact: ")
                        self.print_log(fact)
                    try:
                        self.env.assert_string(fact)
                    except Exception as e:
...
#
    ...
    def run_assert_sim_fact(self, fact_index):
        ...
        try:
            ...
            try:
                if self.control["debug-fact"] == 1:
                    self.print_log("**** DDR Debug: Assert sim FACT index: " + str(fact_index) + " " + str(self.sim_data[int_index]))
    ...
    def run_nc_fact_index(self, index, device_index, num_params, par1, par2, par3, sim_flag=None, sim_index=None, sim_nc=None, sim_nc_index=None):
        ...
        try:
        #
        # if using simulated FACTs from ddr-sim, assert simulated FACT then return 
        #
            if sim_flag == 'TRUE':
                if self.control["debug-fact"] == 1:
                    self.print_log("**** DDR Debug: run_nc_facts_index: assert sim fact index:  " + str(sim_index))
                self.run_assert_sim_fact(sim_index)
                return
        ...
        #
            ...
            if self.control["debug-fact"] == 1:
                self.print_log("\n**** DDR Debug: run_nc_facts_index device_index, NETCONF get RPC: \ndevice_index:" + str(device_index) + " " + str(formatted))
            ...
            #
            # if using simulated netconf responses from ddr-nc, get the simulated response 
            #
                if sim_nc == 'TRUE':
                    self.get_template_multifacts_index(fact["data"], "device", device_id, device_index, sim_nc=True, simdata=self.nc_list[sim_nc_index]["content"])
                else:
            ...
            #
                if sim_nc == 'TRUE':
                    self.get_template_multifacts_protofact_index(fact, "device", device_id, device_index, sim_nc=True, simdata=self.nc_list[sim_nc_index]["content"])
                else:
    ...
    def run_cancel_ping(self, neighbor):
        """
        ...
        try:
            ...
            result = template.assert_fact(**fact1)
            if self.control["debug-fact"] == 1:
                self.print_log("**** DDR Debug: run_cancel_ping assert FACT: " + str(result))
        except Exception as e:
            self.print_log("\n%%%% DDR ERROR: run_cancel_ping error: " + str(e) + "\n")
        return
    ...
    #
                        ...
                        try:
                            ...
                            result = template.assert_fact(**fact1)
                            if self.control["debug-fact"] == 1:
                                self.print_log("**** DDR Debug: run_ping_action assert FACT: " + str(result))
                        except Exception as e:
   ...
   #
    ...
    def run_show_parameter_index_version(self, fact_index, device_index, show_template, pcount, par1, par2, par3, command_timeout=None, version=None, sim_flag=None, sim_index=None, show_flag=None, show_index=None):
        """
        ...
        Usage::
              ...
              (run_show_parameter 3 1 "show monitor capture CAP buffer brief" 0 None None None)
              (run_show_parameter 3 0 "show platform software fed active vp key {0} {1}" 2 ?key ?vlan None)
        ...
        try:
            ...
            self.print_log("\n%%%% DDR ERROR: run_show_parameter_index_version sim-show error: " + str(response) + "\n")
            return

        if self.control["actions"] == 1:
            if self.control["debug-CLI"] == 1:
                self.print_log("\n**** DDR Debug: run_show_parameter_index_version: show_template: " + str(show_template))
            index = int(fact_index)
        ...
        #
        # If show_flag is true, read the simulated show command response from the ddr-show file instead of from device
        #            
            if show_flag == 'TRUE':
                try:
                    if self.control["debug-show"] == 1:
                        self.print_log("**** DDR Debug: run_show_parameter_index_version:  \n" + str(self.show_list[show_index]["content"]))
                    response = self.show_list[show_index]["content"]
                except Exception as e:
                    self.print_log("\n%%%% DDR ERROR: run_show_parameter_index_version sim-show error: " + str(response) + "\n")
                    return
        ...
        #
                    if device_access == 'cli':
                        if device_index == 0:
                            response = cli.cli(command, "30")
                            if self.control["debug-CLI"] == 1:
                                self.print_log("**** DDR Notice: run_show_parameter_index_version CLI Command Executed: " + str(command) + " at: " + str(timestamp))
                                self.print_log("**** DDR Notice: run_show_parameter_index_sersion CLI Command Response: \n" + str(response))
                        else:
        ...
        #
         ...
         #
            ...
            try:
                if self.control["debug-CLI"] == 1:
                    self.print_log("**** DDR Debug: run_show_parameter_index response: " + str(response) + "\n")
                if "error" in response:
                ...
                if parser == None:
                    self.print_log("\n%%%% DDR Error: run_show_parameter_index_version: parser not found: " + str(fact["genie_parser"]) + " \n")
                    return
    #
    # Parse data if the parser is available
    #
                try:
                    ...
                    self.print_log("\n%%%% DDR Error: Exception in run_show_parameter_index_version: parsing error: " + str(e))
                    return
    ...
    def run_cancel_trace(self, neighbor):
        """
        ...
        try:
            ...
            result = template.assert_fact(**fact1)
            if self.control["debug-fact"] == 1:
                self.print_log("**** DDR Debug: run_cancel_trace assert FACT: " + str(result))
        except Exception as e:
            self.print_log("\n%%%% DDR ERROR: run_cancel_trace cancel error: " + str(e) + "\n")
        return

    def run_trace_failed(self, neighbor):
        """
        ...
        try:
            ...
            result = template.assert_fact(**fact1)
            if self.control["debug-fact"] == 1:
                self.print_log("**** DDR Debug: run_trace_failed assert FACT: " + str(result))
        except Exception as e:
            self.print_log("\n%%%% DDR ERROR: ssh run_trace_failed fact assert error: " + str(e) + "\n")
        return
    ...
    #
                        ...
                        try:
                            ...
                            result = template.assert_fact(**fact1)
                            if self.control["debug-fact"] == 1:
                                self.print_log("**** DDR Debug: run_trace assert FACT: " + str(result))
                        except Exception as e:
